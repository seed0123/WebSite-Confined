...
const predefinedChannelIds = ['2573701', '2573811', '2599736'];
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (permit) {
        ...
        setInterval(updateDeviceCoordinates, 1500); // Update device coordinates every 1500ms

        // Load device names from localStorage
        deviceNames = JSON.parse(localStorage.getItem(`devices_${permit}`)) || [];
        ...
        deviceForm.addEventListener('submit', function(event) {
            ...
            localStorage.setItem(`devices_${permit}`, JSON.stringify(deviceNames));
...
async function fetchAndRenderDevices(permit) {
    const channelIds = getChannelIdsForWorkspace(permit);
...
function getChannelIdsForWorkspace(permit) {
    // Return the predefined channel IDs for any workspace
    return predefinedChannelIds;
}
